...
%         session=SESSION; %reload because edited out toE transitions!
    ...
    elseif exp==2
        mat1=mean(cat(3,Etypes.aw1),3);
        eb=std(cat(3,Etypes.aw1),0,3);
        s=subplot(3,3,4),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat1,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
        ...
        mat2=mean(cat(3,Etypes.aw2),3);
        eb=std(cat(3,Etypes.aw2),0,3);
        s=subplot(3,3,5),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat2,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
        ...
        mat3=mean(mat_diff,3);
        eb=std(mat_diff,0,3);
        s=subplot(3,3,6),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat3,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
    ...
    elseif exp==3
        mat1=mean(cat(3,Etypes.aw1),3);
        eb=std(cat(3,Etypes.aw1),0,3);
        s=subplot(3,3,7),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat1,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
        ...
        mat2=mean(cat(3,Etypes.aw2),3);
        eb=std(cat(3,Etypes.aw2),0,3);
        s=subplot(3,3,8),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat2,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
        ...
        mat3=mean(mat_diff,3);
        eb=std(mat_diff,0,3);
        s=subplot(3,3,9),surf(X,Y,Z,'FaceColor','k','FaceAlpha',0.2,'EdgeColor','none');hold on;
        h=barError3d(mat3,eb,eb,faceColor);
        set(s,'FontSize',font);
        yticks([1:3]);xticks([0.5:1:2.5]);grid off
        xlim([0 3]);
    ...
    % lightcycle=12:
    figure(f1);
%     RD(:,end-7:end)=[];zg(:,end-2:end)=[]; %bin 10min win 4h
    RD(:,end-3:end)=[];zg(:,end-2:end)=[];
    % ax(1)=subplot(4,2,2),plot(zg,RD,'o');grid on;hold on;alpha(.3);
%     ax(1)=subplot(4,2,1),fill([zg fliplr(zg)],[nanmean(RD,1)-nanstd(RD,1)/sqrt(n_animals-sum(isnan(RD),1)) fliplr(nanmean(RD,1)+nanstd(RD,1)/sqrt(n_animals-sum(isnan(RD),1)))],'yellow','FaceAlpha',0.3,'LineStyle','--');hold on;
    if exp==1    
...
%     title('Drive switching');
    ...
    % for a=1:size(session,2)
    %     subplot(4,2,4),plot(session(a).Bstart,session(a).Bdur,'o',"MarkerEdgeColor",[0 0 1/a]);hold on;
    %     grid on;
    %     ylim([0 hours(1)]);
    %     for b=1:size(Etimes,2)
    %         subplot(4,2,6),plot(Etimes(b).Etimes(a).Etimes,milliseconds(Tdurs(b).Tdurs(a).Tdurs),'s',"MarkerEdgeColor",[0 0 1/a]);hold on;
    %         alpha(.3);
    %     end
    % end
    % grid on;
...
%     mean_Bdur(:,end-7:end)=[];sd_Bdur(:,end-2:end)=[]; %bin 10min win 4h
    mean_Bdur(:,end-3:end)=[];sd_Bdur(:,end-3:end)=[];
%     ax(3)=subplot(4,2,3),fill([zg fliplr(zg)],[mean_Bdur-seconds(sd_Bdur) fliplr(mean_Bdur+seconds(sd_Bdur))],'cyan','FaceAlpha',0.3,'LineStyle','--');hold on;
    ...
    set(gca,'xtick',[]);
    xlim(hours([17 x_chop]));
%     mean_Tdur(:,end-7:end)=[];sd_Tdur(:,end-2:end)=[]; %bin 10 min win 4h
    mean_Tdur(:,end-3:end)=[];sd_Tdur(:,end-3:end)=[]; 
%     ax(4)=subplot(4,2,5),fill([zg fliplr(zg)],[milliseconds(mean_Tdur)-milliseconds(sd_Tdur) fliplr(milliseconds(mean_Tdur)+milliseconds(sd_Tdur))],'magenta','FaceAlpha',0.3,'LineStyle','--');hold on;
    if exp==1    
